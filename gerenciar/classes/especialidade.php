...
class EspecialidadeController extends ControllerAbstract {
    ...
    function save($metodo=null)
    {
        ...
        $sql = "DELETE FROM tb_especilidade_doencas WHERE tb_especiliadade_id = :id";
        $op = Conexao::getInstance()->prepare($sql);
        $op->bindValue(":id", $especialidade->id);
        $op->execute();

        foreach ($dados['doenca']['id'] as $ks => $vs) :
            $doenca = $doencaModel->BuscarPorCOD($vs,false);
            if($doenca->id > 0):
                $retorno = $this->saveDoencaEspecialidade($doenca->id, $especialidade['id']);
            endif;
        endforeach;
    ...
    */
    ...
    public static function BuscarPorCODMedico($id, $cls=true) {
        try {
            $sql = "SELECT * FROM w18_listamed_dev.tb_medico_especialidade 
