<?php
	...
	class ControllerAbstract extends ClassAbstract {
		...
		public function delete($value = null, $id=true)
		{
			...
			$id_medico = parent::buscaPermissoes($token,$_GET['tb_medico_id']);

			if($id_medico>0):
				$dono = parent::BuscarPorCOD($value->id);

				if($dono->medico_id == $id_medico):
					...
					return array(
						"status" => 403,
						"msg" => "Esta informação pertece a outro usuario, você não tem permissão para executar essa ação.",
					);
				endif;	
			else:
				return array(
