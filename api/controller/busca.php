...
/*		$palavra_array = static::verificaPalavra(static::tirarAcentos($palavra), ' ');
		...
		// se a palavra do banco é composta
		if(is_array($palavra_array)){
			// percorre as palavras que vem do banco 1 a 1
			foreach ($palavra_array as $pa => $va) {
				// se a palavra que deve buscar é composta
				if(is_array($buscar_array)){
					// percorre a palavra 1 por 1
					foreach ($buscar_array as $pb => $vb) {
						// verifica se a palavra tem mais de 3 caracteres
						if(strlen($vb)>3){
							// verifica o levenshtein
							$levAux = levenshtein($va, $vb);
							// verifica se o levenshtein é menor do que o já encontrado
							$lev = ($lev > $levAux) ? $levAux : $lev;
							// se encontrou a palavra exata
							if($levAux==0){
								// se não foi a primeira vez que encontrou a palavra exata
								if($init>0){
									$lev = $lev - 1;
								}
								// contador de palavras exatas
								$init++;
							}
						}
					}
				// se a palavra que deve buscar não for composta
				}else{
					// verifica se ela tem mais de 3 caracteres
					if(strlen($buscar)>3){
						// verifica o levenshtein da palavra
		...
		// se a palavra do banco não é composta
		}else{
			// se a palavra buscar é composta
			if(is_array($buscar_array)){
				// percorre as palavras
				foreach ($buscar_array as $pb => $vb) {
					// se palavra tiver mais que 3 caracteres
					if(strlen($vb)>3){
						// verifica o levenshtein
			...
			// se a palavra buscar não é composta
			}else{
				// verifica se ela tem mais de 3 caracteres
				if(strlen($buscar)>3){
					// verifica o levenshtein
		...
		$palavra_array = static::verificaPalavra(strtolower(static::tirarAcentos($palavra)), ' ');
		$buscar_array  = static::verificaPalavra(static::tirarAcentos($buscar), '-');
	...
	public static function encontrarOcorrencia($info, $buscar)
	{
		...
		$limite = 1;
		$buscar_count  = count(static::verificaPalavra(static::tirarAcentos($buscar), '-'));
		...
		if($tamanho>3):
			...
			if( ($lev<0) && ($buscar_count>1) ){
				$ocorrencias[$k1] = $p1;
			}
			if( ($lev<=$limite) && ($buscar_count==1) ){
				$ocorrencias[$k1] = $p1;
			}
	...
	public static function buscaFullFiltro($cidade) 
	{
		...
		$sql2 = "SELECT 
				    ...
				    tb_consultorio.tb_endereco_id as consultorio_enderecoid,
		...
		foreach ($info as $k => $v) {
			...
			if($v['consultorio_nome']!=""){
				...
				$infoSaida[$idMedico]['consultorio'][$v['consultorio_id']]['consultorio_enderecoid'] = $v['consultorio_enderecoid'];
